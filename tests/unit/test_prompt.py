...
from farmos_ext.cli.prompt import DateValidator, NumberValidator, YesNoValidator, prompt, prompt_date, prompt_number, prompt_option, prompt_yes_no
...
def test_prompt():
    rettext = 'hello'
    with patch('farmos_ext.cli.prompt.prmpt', return_value=rettext) as prmp:
        ret = prompt("Test")
        assert ret == rettext
...
def test_prompt_number():
    retnum = 5
    with patch('farmos_ext.cli.prompt.prmpt', return_value=retnum) as prmp:
...
def test_prompt_yes_no():
    ret = 'y'
    with patch('farmos_ext.cli.prompt.prmpt', return_value=ret) as prmp:
        rets = prompt_yes_no("Opt")
        assert rets

    ret = 'n'
    with patch('farmos_ext.cli.prompt.prmpt', return_value=rets) as prmp:
        rets = prompt_yes_no("Opt")
        assert not rets


def test_prompt_options():
    with patch('farmos_ext.cli.prompt.prmpt', return_value='a'):
        with patch("farmos_ext.cli.prompt.Validator"):
            items = ['a', 'b']
